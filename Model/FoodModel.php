...
class FoodModel extends BaseModel
{
        ...
        public function __construct($food_id = null,  $category_id = null,  $food_name = null,  $description = null,  $price = null,  $image_url = null,  $status = null)
        {
        ...
        public function getAll()
        {
                try {
                        $list_foods = [];
                        $stmt = $this->db->query("SELECT * FROM " . self::TB_NAME . " join category on " . self::TB_NAME . ".category_id = category.category_id WHERE status = 1");
                        $stmt->setFetchMode(PDO::FETCH_ASSOC);
                        while ($row = $stmt->fetch()) {
                                $list_foods[] = new FoodModel($row['food_id'], $row['category_id'], $row['food_name'], $row['description'], $row['price'], $row['image_url'], $row['status']);
                        }
                        return $list_foods;
